...
[![OpenAPI 3.0](https://img.shields.io/badge/Swagger-OpenAPI%203.0-brightgreen?logo=swagger)](https://swagger.io/)
[![JWT](https://img.shields.io/badge/Auth-JWT%20Bearer-orange?logo=jsonwebtokens)](https://jwt.io/)
...
- [Real Project Interface &amp; Previews](#-real-project-interface--previews)
- [System Architecture &amp; UML Design](#-system-architecture--uml-design)
- [Database Design &amp; Data Models](#-database-design--data-models)
  - [1. Conceptual ERD (Chen Notation)](#1-conceptual-erd-chen-notation)
  - [2. Relational Schema (ER-to-Relational Mapping)](#2-relational-schema-er-to-relational-mapping)
...
1. **Hotels &amp; Locations**: Multi-branch support across major destinations (Nile, Red Sea, Alexandria, Luxor, Aswan, etc.).
2. **Buildings &amp; Wings**: Physical architectural breakdown of hotel properties into distinct wings, towers, and beachfront blocks.
3. **Room Inventory &amp; Categorization**: Fine-grained distinction between **Standard Rooms** and **VIP Luxury Suites** with specialized amenities.
4. **Amenities &amp; Components**: Many-to-Many inventory matrix binding rooms with air conditioning, Wi-Fi 6, Jacuzzis, minibars, sound systems, and private balconies.
5. **Reservation Lifecycle**: Automated availability checking, conflict prevention, check-in/check-out date tracking, and cancellation handling.
6. **Role-Based Security**: Cryptographically signed JSON Web Tokens (JWT) securing administration and customer reservation operations.
...
![Hotel Reservation System Portal](docs/portal-dashboard.svg)
...
![Swagger UI API Documentation](docs/swagger-preview.svg)
...
![Entity Relationship Diagram - Chen Notation](docs/database-erd.svg)

#### Entity & Relationship Breakdown:
| Relationship | Between Entities | Cardinality | Business Meaning |
| :--- | :--- | :---: | :--- |
| **has** | `Hotel` &rarr; `Building` | `1 : N` | A hotel possesses one or more distinct buildings or wings. |
| **offers** | `Hotel` &rarr; `Room` | `1 : N` | A hotel offers multiple accommodation rooms. |
| **locatedIn** | `Room` &rarr; `Building` | `N : 1` | Every room is situated within a specific building block. |
| **includes** | `Room` &rarr; `Component` | `M : N` | Rooms can have multiple amenities; amenities are shared across rooms. |
| **hosts** | `Hotel` &rarr; `Reservation` | `1 : N` | A hotel hosts reservations booked by guests. |
| **makes** | `Customer` &rarr; `Reservation` | `1 : N` | A customer places one or more bookings. |
| **for** | `Room` &rarr; `Reservation` | `1 : N` | A specific room is assigned to reservations across time windows. |
| **isVIP** | `Room` &rarr; `VipRoom` | `1 : 1` | Specialty 1-to-1 extension storing VIP suite attributes. |
| **requestedFor** | `VipRoom` &rarr; `LateCheckOutRequest` | `1 : N` | VIP suites allow guests to log late checkout requests. |
...
![Relational Schema Diagram](docs/relational-schema.svg)
...
- **Interactive Swagger Documentation**: [http://localhost:3000/swagger](http://localhost:3000/swagger)
...
| :---: | :--- | :--- | :---: |
| 🧑‍💻 | **Gamal Sameh** (Team Lead) | System Architecture, Database Design &amp; Backend API | [@Gemy21](https://github.com/Gemy21) |
| 🧑‍💻 | **[Team Member Name]** | *[e.g., Database Modeling &amp; SQL Schema]* | [@username](https://github.com/) |
| 🧑‍💻 | **[Team Member Name]** | *[e.g., UML Design &amp; Entity Relationships]* | [@username](https://github.com/) |
| 🧑‍💻 | **[Team Member Name]** | *[e.g., API Controller Development &amp; DTOs]* | [@username](https://github.com/) |
| 🧑‍💻 | **[Team Member Name]** | *[e.g., Frontend Interface &amp; Integration Testing]* | [@username](https://github.com/) |
